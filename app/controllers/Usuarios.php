	...
	class Usuarios{
		...
		public function editarUsuario(){
			...
			if(isset($P_PUT['grupo']) &&
				...
				if((!is_null($usuario[0]['RESULT']) && !empty($usuario[0]['RESULT'])) && $usuario[0]['RESULT'] > 0){
					$mod = explode('|', $P_PUT['mod']);
					$UsuarioMdl->reiniciarModulos($P_PUT['id_usuario']);
					for($i = 0; $i < (count($mod)-1); $i++){
						$UsuarioMdl->guardarModulos($P_PUT['id_usuario'], $mod[$i]);
					}

					$priv = explode('|', $P_PUT['priv']);
					
					$UsuarioMdl->reiniciarPrivilegios($P_PUT['id_usuario']);
					
					for($i = 0; $i < (count($priv)-1); $i++){
						$UsuarioMdl->guardarPrivilegios($P_PUT['id_usuario'], $priv[$i]);
					}
					$res['codigo'] = '200';
